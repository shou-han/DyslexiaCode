...
ET.event = cell2mat(test3); % timing of event start
ET.event(:,2) = 5*ones(size(ET.event,1),1); % set these events to the start of the trial, equate with triggers in EEG

%% find blinks/saccades
eventType1 = 'EBLINK'; % find the blinks
eventType2 = 'ESACC'; % find all the saccade after blink
...
tmp = [tmp1 ; tmp2]; % put blinks and saccades in one column

if isempty(tmp) % if not on the left eye (default), try the right eye
    side=[' R'];
